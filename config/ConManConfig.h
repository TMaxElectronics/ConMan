...
#define CONMAN_SERIALNR_ADDRESS ( ( (uint32_t) ((CONMAN_DATA_ADDRESS) + CONMAN_DATA_SIZE) / 16 ) * 16 ) //byte align to 16 byte-boundaries 
#define CONMAN_EXC_ADRESS       ( ( (uint32_t) (((CONMAN_SERIALNR_ADDRESS) + sizeof(ConMan_SerialNumber_t)) / 16) + 1 ) * 16 ) //byte align to 16 byte-boundaries 
...
#define CONMAN_MEMDESCRIPTOR_ADDRESS 0x9d039000 //( ( (uint32_t) ((CONMAN_DATA_ADDRESS) - sizeof(ConMan_MemoryDescriptor_t)) / 16 ) * 16 )
#define CONMAN_DATA_ADDRESS  ( ( (uint32_t) ((CONMAN_MEMDESCRIPTOR_ADDRESS) + sizeof(ConMan_MemoryDescriptor_t)) / 16 ) * 16 )
